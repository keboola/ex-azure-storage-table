    ...
    public function processItem(object $item, int $rowIndex): void
    {
        ...
        if (!$this->valueTypeChecked) {
            ...
            if ($newType === EdmType::STRING) {
                $this->logger->warning(sprintf(
                    'Warning: Key "%s", which has type "%s", is used for incremental fetching. ' .
                    'Make sure all values are and will be the same length, ' .
        ...
        if ($newType === EdmType::STRING) {
            ...
            if ($this->maxValue && $newValueLength !== $this->maxValueLength) {
                throw new UserException(sprintf(
                    'Unexpected value: Key "%s", which has type "%s", is used for incremental fetching. ' .
                    'In this case, all values must be the same length. ' .
